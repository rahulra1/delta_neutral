...
class IVCrushStrategy(BaseStrategy):
    ...
    def _calc_iv_rv(self, call_option, put_option):
        ...
        if rv_estimate <= 0:
            ...
            logger.warning("⚠ Could not compute realized volatility — skipping IV/RV filter")
    ...
    def initialize(self):
        logger.info("=" * 70)
        logger.info("IV CRUSH / EARNINGS VOLATILITY STRATEGY")
        logger.info("=" * 70)
        logger.info(f"Asset: {self.asset} | Lots: {self.lot_size}")
        logger.info(f"IV/RV Threshold: {self.iv_rv_threshold} | Max Loss: {self.max_loss_pct}% | Target: {self.target_profit_pct}%")
        logger.info("=" * 70)

        # Auto-select best expiry if not provided
        if not self.expiry_date:
            logger.info("[1/5] Auto-selecting best expiry...")
            ...
            for exp in expiries[:3]:
                ...
                logger.info(f"  {exp}: ATM IV = {avg_iv:.4f}")
                if avg_iv > best_iv:
                    best_iv = avg_iv
                    best_expiry = exp
            if not best_expiry:
                logger.warning("✗ Could not find expiry with IV data")
                return False
            self.expiry_date = best_expiry
            logger.info(f"✓ Selected expiry: {self.expiry_date} (IV: {best_iv:.4f})")
        else:
            logger.info(f"[1/5] Using provided expiry: {self.expiry_date}")

        logger.info(f"[2/5] Fetching option chain for {self.expiry_date}...")
        ...
        logger.info("[3/5] Finding ATM options...")
        ...
        if not call_option or not put_option:
            logger.warning("✗ Could not find ATM call/put pair")
            return False
        ...
        logger.info(f"✓ Call: {call_option['symbol']} | Strike: {call_option.get('strike_price')} | ${call_option.get('mark_price', 0):.2f}")
        logger.info(f"✓ Put:  {put_option['symbol']} | Strike: {put_option.get('strike_price')} | ${put_option.get('mark_price', 0):.2f}")
        ...
        logger.info(f"[4/5] IV: {avg_iv:.4f} | RV est: {rv_est:.4f} | IV/RV: {ratio:.2f}")

        if ratio < self.iv_rv_threshold:
            logger.warning(f"✗ IV/RV ratio {ratio:.2f} below threshold {self.iv_rv_threshold}. IV not overpriced enough.")
            self.status_msg = f'IV/RV {ratio:.2f} < {self.iv_rv_threshold} — skipped'
            return False

        logger.info(f"✓ IV is overpriced (ratio {ratio:.2f} >= {self.iv_rv_threshold})")
        ...
        logger.info("=" * 70)
        logger.info("✓ IV CRUSH STRATEGY INITIALIZED")
        logger.info(f"Short Call: {call_option['symbol']} @ ${self.call_entry_price:.2f}")
        logger.info(f"Short Put:  {put_option['symbol']} @ ${self.put_entry_price:.2f}")
        logger.info(f"Total Premium Collected: ${self.total_premium:.2f}")
        logger.info(f"Max Loss: ${self.total_premium * self.max_loss_pct / 100:.2f} | Target: ${self.total_premium * self.target_profit_pct / 100:.2f}")
        logger.info("=" * 70)
    ...
    def monitor(self):
        logger.info(f"[MONITORING] IV Crush — updates every {self.monitoring_interval}s")
        iteration = 0
        try:
            while self.running:
                iteration += 1
                ...
                logger.info(f"[{ts}] #{iteration} | PnL: ${self.total_pnl:.2f} ({pnl_pct:+.1f}%) | IV Crush: {self.iv_crush_pct}%")
                logger.info(f"  Call: ${call_price:.2f} (entry ${self.call_entry_price:.2f}) | Put: ${put_price:.2f} (entry ${self.put_entry_price:.2f})")

                # Exit conditions
                if self.total_premium > 0:
                    # Target profit hit
                    if pnl_pct >= self.target_profit_pct:
                        logger.info(f"🎯 TARGET HIT! PnL: ${self.total_pnl:.2f} ({pnl_pct:.1f}%)")
                        self.close_all()
                        self.status_msg = f'Target hit: ${self.total_pnl:.2f}'
                        break
                    # Max loss hit
                    if pnl_pct <= -self.max_loss_pct:
                        logger.info(f"🛑 MAX LOSS HIT! PnL: ${self.total_pnl:.2f} ({pnl_pct:.1f}%)")
                        self.close_all()
    ...
    def close_all(self):
        ...
        self.running = False
        logger.info(f"✓ Closed | Final PnL: ${self.total_pnl:.2f} | IV Crush: {self.iv_crush_pct}%")
